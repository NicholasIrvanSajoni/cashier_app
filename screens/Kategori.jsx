...
const Kategori = ({ navigation }) => {
    ...
    const [DataListProdukDetailKategori, setDataListProdukDetailKategori] = useState()
    ...
    const fetchKategori = async () => {
        try {
            ...
            const list_kategori_produk = await Async.map(getkategoriproduk.data, async (kategori_data, cb) => {
                const kategori_count = (await axios.get(`/kategori/detail/${kategori_data.id_kategori}`)).data
                cb(null, {
                    ...kategori_data,
    ...
    const handlerSubmitTambahKategori = async () => {
        try {
            if (InputTambahKategori == "") {
                ...
                const tambahktgr = (await axios.post('/kategori',
                    {
    ...
    const handlerEditKategori = async () => {
        try {
            if (InputEditKategori == "") {
                ...
                const editktgr = (await axios.put(`/kategori/${put_id_kategori}`,
                    {
    ...
    const handlerDeletekategori = async () => {
        try {
            const deletektgr = (await axios.delete(`/kategori/${delete_id_kategori}`,
                {
                    headers:
                    {
                        Authorization: 'bearer ' + usercontext.token
    ...
    const fetchProdukKategori = async (id_kategori) => {
        try {
            const get_kategori_produk_by_id = (await axios.get(`/kategori/${id_kategori}`)).data
            ...
            const list_kategori_produk_by_id = await Async.map(get_kategori_produk_by_id.data, async (kategori_produk_data, cb) => {
                const kategori_count = (await axios.get(`/kategori/detail/${kategori_produk_data.id_kategori}`)).data
                cb(null, {
    ...
    const fetchDetailKategoriListProduk = async (id_kategori) => {
        try {
            const detail_kategori_list_produk = (await axios.get(`/product/kategori/${id_kategori}`)).data
            ...
            <View style={kategoriStyle.header}>
                <TouchableOpacity onPress={() => navigation.navigate('Home')}>
                    <View style={kategoriStyle.rightheader}>
                        <Image style={kategoriStyle.leftheader} source={require('../assets/images/Icon/arrowleft.png')}></Image>
                    </View>
                </TouchableOpacity>
                ...
                <View style={kategoriStyle.content}>
                    <View style={kategoriStyle.box_dashboard_kategori}>
                        {DataListKategori?.map((listkategori) => (
                            ...
                            <TouchableOpacity style={kategoriStyle.item_kategori} key={listkategori.id_kategori} onPress={
                                () => {
                                    setShowProdukKategori(true);
                                    fetchProdukKategori(listkategori.id_kategori);
                                    fetchDetailKategoriListProduk(listkategori.id_kategori);
                                }
                            }>
                                <View style={kategoriStyle.box_text_title}>
                                    <Text numberOfLines={2} style={kategoriStyle.title_kategori}>
                                        {listkategori.nama_kategori}
                                    </Text>
                                </View>
                                <View style={kategoriStyle.flex_kategori}>
                                    <View>
                                        <Text style={kategoriStyle.body_kategori}>
                                            Produk : {listkategori.jumlah_produk}
                                        </Text>
                                    </View>
                                    <View style={kategoriStyle.box_action_kategori}>
                                        <TouchableOpacity style={kategoriStyle.box_icon_kategori} onPress={() => {
                                            setput_id_kategori(listkategori.id_kategori);
                                            setPlaceholderNamaKategori(listkategori.nama_kategori)
                                            setShowEditKategori(true);
                                            setDisabled(true);
                                        }}>
                                            <Image style={kategoriStyle.icon_trash} source={require("../assets/images/Icon/edit.png")}></Image>
                                        </TouchableOpacity>
                                        <TouchableOpacity style={kategoriStyle.box_icon_kategori} onPress={() => {
                                            setput_nama_kategori(listkategori.nama_kategori);
                                            setdelete_id_kategori(listkategori.id_kategori);
                                            setShowDeleteKategori(true);
                                            setDisabled(true);
                                        }}>
                                            <Image style={kategoriStyle.icon_trash} source={require("../assets/images/Icon/trash.png")}></Image>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                            </TouchableOpacity>
                        ...
                        <View style={kategoriStyle.content_header_modal}>
                            {DataListDetailKategori?.map((produk_kategori_data) => (
                                <View key={produk_kategori_data.id_kategori}>
                                    <Text style={kategoriStyle.modal_text_header} numberOfLines={2}>{produk_kategori_data.nama_kategori}</Text>
                                    <Text style={kategoriStyle.modal_text_body}>Produk : {produk_kategori_data.jumlah_produk}</Text>
                                </View>
                            ))}
                        </View>
                        <View style={kategoriStyle.detail_kategori_header}>
                            <View style={kategoriStyle.box_content_modal_detail_kategori}>

                                <View style={kategoriStyle.actionbar}>
                                    <View style={kategoriStyle.searchprodukbar}>
                                        <Searchbar style={kategoriStyle.Searchbarstyle}
                                            placeholder="Search"
                            ...
                            <ScrollView>
                                <View>
                                    <View style={kategoriStyle.boxshowproduk}>
                                        ...
                                        {DataListProdukDetailKategori?.map((list_produk_detail_kategori) => (
                                            <View style={kategoriStyle.boxitem} key={list_produk_detail_kategori.id_kategori}>
                                                <View style={kategoriStyle.itemcartboximage}>
                                                    <Image style={kategoriStyle.itemcartimage}
                                                        source={
                                                ...
                                                <View style={kategoriStyle.deskripsiitem}>
                                                    <View>
                                                        <Text style={kategoriStyle.textbold} numberOfLines={4}>{list_produk_detail_kategori.nama_produk}</Text>
                                                    </View>

                                                </View>
                                                <View style={kategoriStyle.box_icon_produk}>
                                                    <TouchableOpacity>
                                                        <Image style={kategoriStyle.icon_produk}
                                                            source={require('../assets/images/Icon/trash.png')}></Image>
                                                    </TouchableOpacity>
                                                    <Button style={kategoriStyle.btn_save} contentStyle={{ flexDirection: 'row-reverse' }} icon={require('../assets/images/Icon/edit.png')}
                    ...
                    <View style={kategoriStyle.modaltambahkategori}>
                        <View style={kategoriStyle.modalheader}>
                            <View style={kategoriStyle.titlemodal}>
                                <Text style={kategoriStyle.cartheadertext}>Tambah Kategori</Text>
                            </View>
                            <TouchableOpacity onPress={() => setShowTambahKategori(false)}>
                        ...
                        <View style={kategoriStyle.modaladdpembelian}>
                            <View style={kategoriStyle.boxmodaltambahpembelian}>
                                ...
                                <Text style={kategoriStyle.boxheight}>Nama Kategori :</Text>
                                <TextInput style={kategoriStyle.boxinput}
                                    value={InputTambahKategori}
                                    mode={'outlined'}
                                    onChangeText={InputTambahKategori => setInputTambahKategori(InputTambahKategori)}
                                />

                                <Button style={kategoriStyle.btn_save} mode="contained" onPress={() => {
...

            {
                ShowEditKategori ? (
                    <View style={kategoriStyle.modaltambahkategori}>
                        <View style={kategoriStyle.modalheader}>
                            <View style={kategoriStyle.titlemodal}>
                                <Text style={kategoriStyle.cartheadertext}>Edit Kategori</Text>
                            </View>
                            <TouchableOpacity onPress={() => {
                                setShowEditKategori(false)
                                setInputEditKategori("")
                        ...
                        <View style={kategoriStyle.modaladdpembelian}>
                            <View style={kategoriStyle.boxmodaltambahpembelian}>
                                ...
                                <TextInput style={kategoriStyle.boxinput}
                                    ...
                                    onChangeText={InputEditKategori => setInputEditKategori(InputEditKategori)}
                                />
...

            {
                ShowDeleteKategori ? (
                    <View style={kategoriStyle.modaltambahkategori}>
                        <View style={kategoriStyle.modalheader}>
                            <View>
                                <Text style={kategoriStyle.title_delete}>Delete Kategori</Text>
                            </View>
                        </View>
                        <View style={kategoriStyle.modaladdpembelian}>
                            <View style={kategoriStyle.boxmodaltambahpembelian}>
                                ...
                                <View>
                                    <Text style={kategoriStyle.intro_delete}>Apakah anda ingin Menghapus kategori <Text style={kategoriStyle.content_delete}>{put_nama_kategori}</Text> ?</Text>
                                </View>

                                <View style={kategoriStyle.action_delete}>
                                    <Button style={kategoriStyle.btn_del} mode="contained" onPress={() => setShowDeleteKategori(false)}>
                                        Tidak
                                    </Button>
                                    <Button style={kategoriStyle.btn_del} mode="contained" onPress={() => {
            ...
            <TouchableOpacity onPress={() => setShowTambahKategori(true)}>
                <View style={kategoriStyle.btn_round_box}>
                    <View style={kategoriStyle.btn_round}>
                        <Image style={kategoriStyle.btn_round_icon} source={require('../assets/images/Icon/plus.png')}></Image>
                    </View>
                </View>
            </TouchableOpacity>
        </View>
